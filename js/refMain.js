...
// Vue
	var MainVue = new Vue({
		...
		data: {
			...
			browser: {
				key: "f30482c200524217a01b1fd692ca7e92",
		...
		// Setup the recognizer
			RecognizerSetup: function() {
				switch (MainVue.browser.recognitionMode) {
					case "Interactive":
						MainVue.browser.recognitionMode = MainVue.browser.SDK.RecognitionMode.Interactive;
						break;
					case "Conversation":
						MainVue.browser.recognitionMode = MainVue.browser.SDK.RecognitionMode.Conversation;
						break;
					case "Dictation":
						MainVue.browser.recognitionMode = MainVue.browser.SDK.RecognitionMode.Dictation;
						break;
					default:
						MainVue.browser.recognitionMode = MainVue.browser.SDK.RecognitionMode.Interactive;
				}

				MainVue.browser.recognizerConfig = new SDK.RecognizerConfig(
					new SDK.SpeechConfig(
						new SDK.Context(
							new SDK.OS(navigator.userAgent, "Browser", null),
							new SDK.Device("SpeechSample", "SpeechSample", "1.0.00000"))),
					MainVue.browser.recognitionMode,
					MainVue.browser.language, // Supported languages are specific to each recognition mode. Refer to docs.
					MainVue.browser.format); // SDK.SpeechResultFormat.Simple (Options - Simple/Detailed)

				var useTokenAuth = false;

				var authentication = function() {
					if (!useTokenAuth)
						return new SDK.CognitiveSubscriptionKeyAuthentication(MainVue.browser.key);
				...
				if (!files.length) {
					return MainVue.browser.SDK.CreateRecognizer(MainVue.browser.recognizerConfig, authentication);
				} 
				else {
					return MainVue.browser.SDK.CreateRecognizerWithFileAudioSource(MainVue.browser.recognizerConfig, authentication, files[0]);
				}
			},
		// Start the recognition
			RecognizerStart: function() {
				MainVue.browser.recognizer.Recognize((event) => {
					switch (event.Name) {
						case "RecognitionTriggeredEvent":
							MainVue.UpdateStatus("Initializing");
							break;
						case "ListeningStartedEvent":
							MainVue.UpdateStatus("Listening");
							break;
						case "RecognitionStartedEvent":
							MainVue.UpdateStatus("Listening_Recognizing");
							break;
						case "SpeechStartDetectedEvent":
							MainVue.UpdateStatus("Listening_DetectedSpeech_Recognizing");
							// console.log(JSON.stringify(event.Result));
							break;
						case "SpeechHypothesisEvent":
							MainVue.UpdateRecognizedHypothesis(event.Result.Text, false);
							// console.log(JSON.stringify(event.Result));
							break;
						case "SpeechFragmentEvent":
							MainVue.UpdateRecognizedHypothesis(event.Result.Text, true);
							// console.log(JSON.stringify(event.Result));
							break;
						case "SpeechEndDetectedEvent":
							MainVue.OnSpeechEndDetected();
							MainVue.UpdateStatus("Processing_Adding_Final_Touches");
							// console.log(JSON.stringify(event.Result));
							break;
						case "SpeechSimplePhraseEvent":
							MainVue.UpdateRecognizedPhrase(JSON.stringify(event.Result, null, 3));
							MainVue.CallAPI(JSON.stringify(event.Result, null, 3));
							break;
		...
		// Stop the Recognition.
			RecognizerStop: function() {
				MainVue.browser.recognizer.AudioSource.TurnOff();
			},
		// FilePicker on change
			changeFilePicker: function() {
				MainVue.Setup();
				MainVue.browser.hypothesisDiv.innerHTML = "";
				MainVue.browser.phraseDiv.innerHTML = "";
				MainVue.RecognizerStart(MainVue.browser.SDK, MainVue.browser.recognizer);
				document.getElementById("startBtn").disabled = true;
				document.getElementById("stopBtn").disabled = false;
				MainVue.browser.filePicker = "";
			},
		// StopBtn on click
			clickStopBtn: function() {
				MainVue.RecognizerStop(MainVue.browser.SDK, MainVue.browser.recognizer);
				document.getElementById("startBtn").disabled = false;
				document.getElementById("stopBtn").disabled = true;
			},
		// StartBtn on click
			clickStartBtn: function() {
				if (MainVue.browser.key == "" || MainVue.browser.key == "YOUR_BING_SPEECH_API_KEY") {
					console.log("Please enter your Bing Speech subscription key!");
					return;
				}
				// if (MainVue.browser.inputSource === "File") {
				// 	MainVue.browser.filePicker.click();
				// } else {
					if (!MainVue.browser.recognizer || MainVue.browser.previousSubscriptionKey != MainVue.browser.key) {
						MainVue.browser.previousSubscriptionKey = MainVue.browser.key;
						MainVue.Setup();
					}

					document.getElementById("hypothesisDiv").innerHTML = "";
		...
		// key on focus
			focusKey: function() {
				if (MainVue.browser.key == "YOUR_BING_SPEECH_API_KEY") {
					MainVue.browser.key = "";
				}
			},
		// key on focusout
			focusOutKey: function() {
				if (MainVue.browser.key == "") {
					MainVue.browser.key = "YOUR_BING_SPEECH_API_KEY";
				}
			},
			OnComplete: function() {
				MainVue.browser.SDK = window.SDK;
				document.getElementById("startBtn").disabled = false;
				document.getElementById("stopBtn").disabled = true;
			},
			UpdateRecognizedPhrase: function(json) {
				document.getElementById("hypothesisDiv").innerHTML = "";
				document.getElementById("phraseDiv").innerHTML += json + "\n";
			},
			OnSpeechEndDetected: function() {
				document.getElementById("stopBtn").disabled = true;
			},
			Setup: function() {
				if (MainVue.browser.recognizer != null) {
					MainVue.RecognizerStop(MainVue.browser.SDK, MainVue.browser.recognizer);
				}
				else
					MainVue.browser.recognizer = MainVue.RecognizerSetup(MainVue.browser.SDK, MainVue.browser.recognitionMode, MainVue.browser.languageOptions, MainVue.browser.SDK.SpeechResultFormat[MainVue.browser.formatOptions], MainVue.browser.key);
			},
			...
			UpdateRecognizedHypothesis: function(text, append) {
				if (append)
					document.getElementById("hypothesisDiv").innerHTML += text + " ";
				else
					document.getElementById("hypothesisDiv").innerHTML = text;

				var length = document.getElementById("hypothesisDiv").innerHTML.length;
				if (length > 403) {
					document.getElementById("hypothesisDiv").innerHTML = "..." + document.getElementById("hypothesisDiv").innerHTML.substr(length - 400, length);
				}
			},
			CallAPI: function(json) {
				...
				if (MainVue.session.search == "name") {
					var url = "https://test-webapi.mitalent.org/pathfinder/api/Schools?$inlinecount=allpages&$filter=substringof('" + txtFormatted + "', InstName1)%20eq%20true";
				}
				else {
					var url = "https://test-webapi.mitalent.org/pathfinder/api/Schools?$inlinecount=allpages&$filter=City%20eq%20%27" + txtFormatted + "%27";
				}

				request.onreadystatechange = function () {
					if (this.readyState === 4 && this.status === 200) {
						let response = JSON.parse(this.responseText);
						MainVue.foundSchools = [];
						if (response.value.length > 0) {
							response.value.forEach(function(item) {
								MainVue.foundSchools.push(item);
							});

							try {
								bingClientTTS.synthesize("The top 3 results are:");
								bingClientTTS.synthesize(MainVue.foundSchools[0].InstName1);
								bingClientTTS.synthesize(MainVue.foundSchools[1].InstName1);
								bingClientTTS.synthesize("and");
								bingClientTTS.synthesize(MainVue.foundSchools[2].InstName1);
							} catch (error) {
								bingClientTTS.synthesize("Speech Bot ran into an error. Please try again.");
							}
						}
						else
							bingClientTTS.synthesize("No Results Found.");
						
						MainVue.searching = false;
					}
				}
				...
				MainVue.searching = true;
			},
			TestCallAPI: function() {
				var jsonSend = '{"DisplayText":"' + MainVue.testing + '"}';
				MainVue.CallAPI(jsonSend);
			},
			Initialize: function() {
				// if (!!window.SDK) {
				// 	document.getElementById('content').style.display = 'block';
				// 	document.getElementById('warning').style.display = 'none';
				// 	MainVue.onComplete(window.SDK);
				// }
				MainVue.OnComplete(window.SDK);
				MainVue.Setup();
			},
			schoolClick: function(school) {
				window.open("https://" + school.URL, '_blank');
			}
		},
		mounted() {
			document.addEventListener("DOMContentLoaded", function() {
				MainVue.Initialize(function(speechSdk) {
					MainVue.browser.SDK = speechSdk;
					document.getElementById("startBtn").disabled = false;
				});
			});
		}
...
// For Text-To-Speach
			...
			*/
			...
			_getBingSpeechData(text, locale, outputFormat, speechItemIndex) {
				return __awaiter(this, void 0, void 0, function* () {
					...
					try {
						var blobReponse = yield this._tools.makeHttpRequest("POST", "https://westus.tts.speech.microsoft.com/cognitiveservices/v1", true, optionalHeaders, SSML);
					}
					catch (ex) {
	...
	// For Bing Text-To-Speech
	var bingClientTTS = new BingSpeech.TTSClient("e2c1a12891a742c48e0688d4231cd247");